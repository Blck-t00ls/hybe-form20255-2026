...
const SQLITE_DB_PATH = process.env.NODE_ENV === 'production'
  ? '/tmp/otp_database.db'
...
class OTPDatabase {
  ...
  async initialize() {
    ...
    try {
      ...
      console.error('Failed to ensure SQLite directory exists:', e);
    }
  ...
  async createTables() {
    ...
    await new Promise((resolve, reject) => {
      ...
      this.db.run(createOTPTable, (err) => {
        ...
        let pending = createIndexes.length;
        let errors = [];
        createIndexes.forEach((q) => {
          this.db.run(q, (e) => {
            if (e) errors.push(e);
            if (--pending === 0) {
              if (errors.length) console.warn('Index creation warnings:', errors);
              resolve();
            }
          });
        });
